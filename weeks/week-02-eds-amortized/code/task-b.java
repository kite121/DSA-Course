...
public class Main {
    public static void main(String[] args) {
        ...
        ListStack<Integer> stack_evaluation = new ListStack<>();
        while(queue.size() != 1){
            if (skip != 0) {
                skip -= 1;
                continue;
            ...
            if (queue.peek_first() == '+'){
                int first = stack_evaluation.peek();
                stack_evaluation.pop();
                int second = stack_evaluation.peek();
                stack_evaluation.pop();
                stack_evaluation.push(first + second);
                queue.pool();
                continue;
            }
            if (queue.peek_first() == '-'){
                ...
                int second = stack_evaluation.peek();
                stack_evaluation.pop();
                stack_evaluation.push(second - first);
                queue.pool();
                continue;
            }
            if (queue.peek_first() == '*'){
            ...
            if (queue.peek_first() == 'm'){
                queue.pool();
                if (queue.peek_first() == 'i'){
                    int first = stack_evaluation.peek();
                    stack_evaluation.pop();
                    int second = stack_evaluation.peek();
                    stack_evaluation.pop();
                    if (first > second) {
                        stack_evaluation.push(second);
                    }
                    else{
                        stack_evaluation.push(first);
                    }
                }
                else{
                    int first = stack_evaluation.peek();
            ...
            String str = "";
            while(queue.peek_first() != ' '){
                str += queue.peek_first().toString();
                queue.pool();
            }
            stack_evaluation.push(Integer.parseInt(str));
        }
        System.out.println(stack_evaluation.peek());
    }
}
interface Stack<T> {
    void push(T value);
